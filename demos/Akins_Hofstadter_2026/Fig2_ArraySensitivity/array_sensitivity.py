...
ax.plot(1e2*spc.c/(f*1e9), resolution[:,  1], color='k', label='Cassini-like', ls='--')

tax.plot(1e2*spc.c/(f*1e9), resolution[:,  0], color='k', label='Juno-like')
tax.plot(1e2*spc.c/(f*1e9), resolution[:,  1], color='k', label='Cassini-like', ls='--')
ax.set_xlabel('Wavelength (cm)')
ax.set_ylabel('Planetographic spatial resolution (degrees)')
ax.set_title(r"Achievable resolution for $\tau=$10 hr, $\Delta T_B=$0.5 K"+"\n"+ "Saturn")
...
tax.set_title(r"Achievable resolution for $\tau=$10 hr, $\Delta T_B =$0.5 K"+"\n"+ "Uranus")
...
fig.savefig('gb_mwr_comparison_saturn.png', dpi=300)
fig2.savefig('gb_mwr_comparison_uranus.png', dpi=300)
