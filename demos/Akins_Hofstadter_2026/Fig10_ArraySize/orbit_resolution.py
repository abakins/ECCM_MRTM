...
dray = xr.DataArray(
    inst_diam,
    coords={"wavelength": wave, "distance": dist / rad, "resolution": resolution},
    name="width",
)
...
dframe = dframe.rename(
    columns={
        ...
        "distance": "Orbiter distance (R$_P$)",
        "resolution": "Latitude res. (deg.)",
...
sns.lineplot(
    ...
    x="Wavelength (m)",
    y="Aperture width (m)",
    style="Orbiter distance (R$_P$)",
    hue="Latitude res. (deg.)",
    size="Latitude res. (deg.)",
    ax=ax,
...
dray = xr.DataArray(
    resolution.squeeze(),
    coords={"wavelength": wave, "distance": dist / rad},
    name="resolution",
)
dframe = dray.to_dataframe().reset_index()
dframe["wavelength"] = np.round(dframe["wavelength"] * 1e2, decimals=1)
