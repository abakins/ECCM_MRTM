...
class molecule:
    ...
    def absorption(
        self, frequency, gases, gases_index, temperature, pressure, units="dBperkm"
    ):
        ...
        alpha_max = self.absorption_coeff(
            ...
            temperature,
            self.lines["ELO"].ravel(),
            gases[gases_index.index(self.__class__.__name__), :]
            * pressure
            * spc.bar
            / spc.torr,
            gamma_total,
        ...
        if self.lineshape_type == "Van Vleck-Weisskopf":
            lineshape = self.vanvleckweisskopf(
                frequency, self.lines["FREQ"].ravel(), gamma
            )
            ...
            lineshape = self.benreuven(
                frequency, self.lines["FREQ"].ravel(), gamma, zeta, delta
            )
        else:
            raise ValueError(
                "Unspecified lineshape type. Valid options are 'Van Vleck-Weisskopf', 'Gross', or 'Ben-Reuven'"
            )
    ...
    def load_line_parameters(self, temperature, gas_pressure, gases_index):
        ...
        for i in index:
            gamma += self.linewidth_func(
                self.parameters[i][:, self.parameter_labels.index("GAMMA")],
                gas_pressure[gases_index.index(i)],
                self.To / temperature,
                self.parameters[i][:, self.parameter_labels.index("GAMMA_TEMP")],
            )

        if self.lineshape_type == "Ben-Reuven":
            for i in index:
                zeta += self.linewidth_func(
                    self.parameters[i][:, self.parameter_labels.index("ZETA")],
                    gas_pressure[gases_index.index(i)],
                    self.To / temperature,
                    self.parameters[i][:, self.parameter_labels.index("ZETA_TEMP")],
                )
    ...
    def benreuven(frequency, f_o, gamma, zeta, delta):
        ...
        shape = (
            ...
            / pi
            * (f / fo) ** 2
            * ((gam - zet) * f**2 + (gam + zet) * ((fo + delt) ** 2 + gam**2 - zet**2))
            / ((f**2 - (fo + delt) ** 2 - gam**2 + zet**2) ** 2 + 4 * f**2 * gam**2)
        )
        shape = np.moveaxis(shape, -1, 0)
        return shape
    ...
    def import_jpl_file(filename):
        ...
        if lines[0][51:55].strip(" ") == "1404":
            ...
            raise ValueError(
                "Either the input file is not formatted properly or this type of catalog file is not currently supported for import"
            )
...
class PH3(molecule):
    ...
    def write_line_parameters(self):
        ...
        three_mask = (
            (abs(lines["LAM_UPPER"]) == 3).flatten()
            & (abs(lines["LAM_LOWER"]) == 3).flatten()
            & elastic_mask
        )
        six_mask = (
            (abs(lines["LAM_UPPER"]) == 6).flatten()
            & (abs(lines["LAM_LOWER"]) == 6).flatten()
            & elastic_mask
        )
        group1 = (six_mask | three_mask) & (abs(lines["N_UPPER"]) < 8).flatten()
        group2 = three_mask & (
            (abs(lines["N_UPPER"]) >= 8).flatten()
            & (abs(lines["N_UPPER"]) <= 26).flatten()
        )
        group3 = (~(group1 | group2)) & elastic_mask

        ph3[inelastic_mask, 0] = 4.2157 * (
            1e3 * spc.torr / spc.bar
        )  # Converting to MHz/torr from GHz/bar
...
class H2S(molecule):
    ...
    def write_line_parameters(self):
        ...
        h2s[np.floor(lines["FREQ"] / 1e3).flatten() == 168, 0] = 5.38 * (
            1e3 * spc.torr / spc.bar
        )
        h2s[np.floor(lines["FREQ"] / 1e3).flatten() == 216, 0] = 6.82 * (
            1e3 * spc.torr / spc.bar
        )
        h2s[np.floor(lines["FREQ"] / 1e3).flatten() == 300, 0] = 5.82 * (
            1e3 * spc.torr / spc.bar
        )
        h2s[np.floor(lines["FREQ"] / 1e3).flatten() == 393, 0] = 5.08 * (
            1e3 * spc.torr / spc.bar
        )
...
class H2O(molecule):
    ...
    def absorption(
        ...
        line_absorption = super().absorption(
            frequency, gases, gases_index, temperature, pressure, units=units
        )
        ...
        if self.model == "KarpowiczSteffes":
            ...
            continuum_absorption = (
                ...
                * (gp_H2O * 1e3) ** 2
                + 2.10003048186e-31
                * (f / 1e3) ** 2
                * (300 / t) ** 0.0435525417274
                * (gp_H2O * 1e3) ** 6.76418487001
            )
        elif self.model == "BellottiSteffes":
            # From Bellotti et al. 2016
            continuum_absorption = (
                3.1e-12 * (f / 1e3) ** 2 * (300 / t) ** 12 * (gp_H2O * 1e3) ** 2
            )
            ...
            raise ValueError(
                'Specify continuum model as "KarpowiczSteffes", "BellottiSteffes", or "HodgesModified"'
            )
        ...
        if "N2" in gases_index:
            ...
            continuum_absorption += (
                ...
                * (f / 1e3) ** 2
                / 239**2
                * (296 / t) ** (4.86)
                * (gp_N2 * spc.bar / spc.torr / 750)
                * (gp_H2O * spc.bar / spc.torr)
            )
        ...
        if self.model == "HodgesModified":
            ...
            scale[(pressure > P_top) & (pressure < P_bot)] = (mid_pressure - P_top) / (
                P_bot - P_top
            )
    ...
    def write_line_parameters(self):
        ...
        ) / (self.hc / spc.k / self.To)
        ks_h2o_gamma = np.array(
            [
                13.49,
                14.66,
                10.57,
        ...
        h2o[
            np.searchsorted(
                np.round(lines["FREQ"].flatten() / 1e3, 4), bauer_frequency
            ),
            0,
        ] = bauer_h2o_gamma
        h2o[
            np.searchsorted(np.round(lines["FREQ"].flatten() / 1e3, 4), ks_frequency), 0
        ] = ks_h2o_gamma
        ...
        h2o[
            ...
            1,
        ] = bauer_h2o_gamma_temp
        h2o[
            np.searchsorted(np.round(lines["FREQ"].flatten() / 1e3, 4), ks_frequency), 1
        ] = ks_h2o_gamma_temp
...
class NH3(molecule):
    ...
    def __init__(self, model="BellottiSteffes", **kwargs):
        ...
        if self.model == "HanleySteffes" or self.model == "HodgesModified":
            self.inv_lines = sio.loadmat(
                os.path.join(self.line_path, "nh3", "nh3_inversion_HS.mat")
            )
            self.inv_parameters = sio.loadmat(
                os.path.join(self.lineshape_path, "NH3_inversion_HS.mat")
            )
            self.inv_parameter_labels = [
                x.strip(" ") for x in list(self.inv_parameters["labels"])
            ]
        elif self.model == "DevarajSteffes":
            self.inv_lines_lp = sio.loadmat(
                os.path.join(self.line_path, "nh3", "nh3_inversion_DS_lowpressure.mat")
            )
            self.inv_parameters_lp = sio.loadmat(
                os.path.join(self.lineshape_path, "NH3_inversion_DS_lowpressure.mat")
            )
            self.inv_parameter_lp_labels = [
                x.strip(" ") for x in list(self.inv_parameters_lp["labels"])
            ]
            self.inv_lines_hp = sio.loadmat(
                os.path.join(self.line_path, "nh3", "nh3_inversion_DS_highpressure.mat")
            )
            self.inv_parameters_hp = sio.loadmat(
                os.path.join(self.lineshape_path, "NH3_inversion_DS_highpressure.mat")
            )
            self.inv_parameter_hp_labels = [
                x.strip(" ") for x in list(self.inv_parameters_hp["labels"])
            ]
            self.rot_lines = sio.loadmat(
                os.path.join(self.line_path, "nh3", "nh3_rotational_DS.mat")
            )
            self.rot_parameters = sio.loadmat(
                os.path.join(self.lineshape_path, "NH3_rotational_DS.mat")
            )
            self.rot_parameter_labels = [
                x.strip(" ") for x in list(self.rot_parameters["labels"])
            ]
            self.vib_lines = sio.loadmat(
                os.path.join(self.line_path, "nh3", "nh3_rotovibrational_DS.mat")
            )
            self.vib_parameters = sio.loadmat(
                os.path.join(self.lineshape_path, "NH3_rotovibrational_DS.mat")
            )
            self.vib_parameter_labels = [
                x.strip(" ") for x in list(self.vib_parameters["labels"])
            ]
        elif self.model == "BellottiSteffes":
            self.inv_lines_lf = sio.loadmat(
                os.path.join(self.line_path, "nh3", "nh3_inversion_BS_lowfrequency.mat")
            )
            self.inv_parameters_lf = sio.loadmat(
                os.path.join(self.lineshape_path, "NH3_inversion_BS_lowfrequency.mat")
            )
            self.inv_parameter_lf_labels = [
                x.strip(" ") for x in list(self.inv_parameters_lf["labels"])
            ]
            self.inv_lines_hf = sio.loadmat(
                os.path.join(
                    self.line_path, "nh3", "nh3_inversion_BS_highfrequency.mat"
                )
            )
            self.inv_parameters_hf = sio.loadmat(
                os.path.join(self.lineshape_path, "NH3_inversion_BS_highfrequency.mat")
            )
            self.inv_parameter_hf_labels = [
                x.strip(" ") for x in list(self.inv_parameters_hf["labels"])
            ]
            self.rot_lines = sio.loadmat(
                os.path.join(self.line_path, "nh3", "nh3_rotational_BS.mat")
            )
            self.rot_parameters = sio.loadmat(
                os.path.join(self.lineshape_path, "NH3_rotational_BS.mat")
            )
            self.rot_parameter_labels = [
                x.strip(" ") for x in list(self.rot_parameters["labels"])
            ]
            self.vib_lines = sio.loadmat(
                os.path.join(self.line_path, "nh3", "nh3_rotovibrational_BS.mat")
            )
            self.vib_parameters = sio.loadmat(
                os.path.join(self.lineshape_path, "NH3_rotovibrational_BS.mat")
            )
            self.vib_parameter_labels = [
                x.strip(" ") for x in list(self.vib_parameters["labels"])
            ]
            ...
            raise ValueError(
                'Specify model as "HanleySteffes", "DevarajSteffes", "BellottiSteffes", or "HodgesModified"'
            )
    ...
    def absorption(
        ...
        if self.model == "HanleySteffes" or self.model == "HodgesModified":
            ...
            alpha_max = self.absorption_coeff(
                10 ** self.inv_lines["LGINT"].ravel(),
                temperature,
                self.inv_lines["ELO"].ravel(),
                gases[np.array(gases_index) == self.__class__.__name__, :]
                * pressure
                * spc.bar
                / spc.torr,
                gamma,
            )
            inv_lineshape = self.benreuven(
                frequency, self.inv_lines["FREQ"].ravel(), gamma, zeta, delta
            )
            ...
            hp_mask = pressure >= 15
            lp_mat_mask = (
                np.zeros((len(frequency), len(pressure)), dtype=bool) + lp_mask
            )
            hp_mat_mask = (
                np.zeros((len(frequency), len(pressure)), dtype=bool) + hp_mask
            )
            ...
            if low_pressure.size > 0:
                gamma_lp, zeta_lp, delta_lp = self.load_line_parameters(
                    low_temperature,
                    low_gas_pressure,
                    gases_index,
                    self.inv_parameters_lp,
                    self.inv_parameter_lp_labels,
                )
                alpha_max_lp = self.absorption_coeff(
                    10 ** self.inv_lines_lp["LGINT"].ravel(),
                    low_temperature,
                    self.inv_lines_lp["ELO"].ravel(),
                    low_gas_pressure[gases_index.index("NH3")],
                    gamma_lp,
                )
                inv_lp_lineshape = self.benreuven(
                    frequency,
                    self.inv_lines_lp["FREQ"].ravel(),
                    gamma_lp,
                    zeta_lp,
                    delta_lp,
                )
                absorption_lp = np.sum(
                    alpha_max_lp * spc.pi * gamma_lp * inv_lp_lineshape, axis=-1
                )
                np.place(absorption, lp_mat_mask, absorption_lp)
            if high_pressure.size > 0:
                gamma_hp, zeta_hp, delta_hp = self.load_line_parameters(
                    high_temperature,
                    high_gas_pressure,
                    gases_index,
                    self.inv_parameters_hp,
                    self.inv_parameter_hp_labels,
                )
                alpha_max_hp = self.absorption_coeff(
                    10 ** self.inv_lines_hp["LGINT"].ravel(),
                    high_temperature,
                    self.inv_lines_hp["ELO"].ravel(),
                    high_gas_pressure[gases_index.index("NH3")],
                    gamma_hp,
                )
                inv_hp_lineshape = self.benreuven(
                    frequency,
                    self.inv_lines_hp["FREQ"].ravel(),
                    gamma_hp,
                    zeta_hp,
                    delta_hp,
                )
                absorption_hp = np.sum(
                    alpha_max_hp * spc.pi * gamma_hp * inv_hp_lineshape, axis=-1
                )
                np.place(absorption, hp_mat_mask, absorption_hp)

            gamma_rot = self.load_line_parameters(
                temperature,
                gas_pressure,
                gases_index,
                self.rot_parameters,
            ...
            gamma_vib = self.load_line_parameters(
                temperature,
                gas_pressure,
                gases_index,
                self.vib_parameters,
            ...
            alpha_max_rot = self.absorption_coeff(
                10 ** self.rot_lines["LGINT"].ravel(),
                temperature,
                self.rot_lines["ELO"].ravel(),
                gases[gases_index.index("NH3")] * pressure * spc.bar / spc.torr,
                gamma_rot,
            )
            alpha_max_vib = self.absorption_coeff(
                10 ** self.vib_lines["LGINT"].ravel(),
                temperature,
                self.vib_lines["ELO"].ravel(),
                gases[gases_index.index("NH3")] * pressure * spc.bar / spc.torr,
                gamma_vib,
            )

            rot_lineshape = self.gross(
                frequency, self.rot_lines["FREQ"].ravel(), gamma_rot
            )
            vib_lineshape = self.gross(
                frequency, self.vib_lines["FREQ"].ravel(), gamma_vib
            )
            absorption += np.sum(
                alpha_max_rot * spc.pi * gamma_rot * rot_lineshape, axis=-1
            )
            absorption += np.sum(
                alpha_max_vib * spc.pi * gamma_vib * vib_lineshape, axis=-1
            )

        elif self.model == "BellottiSteffes":
            lf_mask = frequency <= 30e3
            hf_mask = frequency > 30e3
            lf_mat_mask = np.transpose(
                np.zeros((len(pressure), len(frequency)), dtype=bool) + lf_mask
            )
            hf_mat_mask = np.transpose(
                np.zeros((len(pressure), len(frequency)), dtype=bool) + hf_mask
            )
            low_frequency = frequency[lf_mask]
            high_frequency = frequency[hf_mask]

            if low_frequency.size > 0:
                gamma_lf, zeta_lf, delta_lf = self.load_line_parameters(
                    temperature,
                    gas_pressure,
                    gases_index,
                    self.inv_parameters_lf,
                    self.inv_parameter_lf_labels,
                )
                alpha_max_lf = self.absorption_coeff(
                    10 ** self.inv_lines_lf["LGINT"].ravel(),
                    temperature,
                    self.inv_lines_lf["ELO"].ravel(),
                    gas_pressure[gases_index.index("NH3")],
                    gamma_lf,
                )
                inv_lf_lineshape = self.benreuven(
                    low_frequency,
                    self.inv_lines_lf["FREQ"].ravel(),
                    gamma_lf,
                    zeta_lf,
                    delta_lf,
                )
                absorption_lf = np.sum(
                    alpha_max_lf * spc.pi * gamma_lf * inv_lf_lineshape, axis=-1
                )
                np.place(absorption, lf_mat_mask, absorption_lf)
            if high_frequency.size > 0:
                gamma_hf, zeta_hf, delta_hf = self.load_line_parameters(
                    temperature,
                    gas_pressure,
                    gases_index,
                    self.inv_parameters_hf,
                    self.inv_parameter_hf_labels,
                )
                alpha_max_hf = self.absorption_coeff(
                    10 ** self.inv_lines_hf["LGINT"].ravel(),
                    temperature,
                    self.inv_lines_hf["ELO"].ravel(),
                    gas_pressure[gases_index.index("NH3")],
                    gamma_hf,
                )
                inv_hf_lineshape = self.benreuven(
                    high_frequency,
                    self.inv_lines_hf["FREQ"].ravel(),
                    gamma_hf,
                    zeta_hf,
                    delta_hf,
                )
                absorption_hf = np.sum(
                    alpha_max_hf * spc.pi * gamma_hf * inv_hf_lineshape, axis=-1
                )
        ...
        if self.model == "HodgesModified":
            ...
            scale_factor[(pressure > P_top) & (pressure < P_bot)] = (
                1 - scale
            ) + scale * f_bot
            absorption = absorption * scale_factor
    ...
    def load_line_parameters(
        self, temperature, pressure, gases_index, parameters, parameter_labels
    ):
        ...
        if self.model == "HanleySteffes" or self.model == "HodgesModified":
            ...
            for i in index:
                if i == "NH3":
                    self.To = 295
                    gamma += self.linewidth_func(
                        parameters[i][:, parameter_labels.index("GAMMA")],
                        pressure[gases_index.index(i), :],
                        self.To / temperature,
                        parameters[i][:, parameter_labels.index("GAMMA_TEMP")],
                    )
                    zeta += self.linewidth_func(
                        parameters[i][:, parameter_labels.index("ZETA")],
                        pressure[gases_index.index(i), :],
                        self.To / temperature,
                        parameters[i][:, parameter_labels.index("ZETA_TEMP")],
                    )
                    self.To = 300
                else:
                    gamma += self.linewidth_func(
            ...
            if (
                parameters is self.inv_parameters_lp
                or parameters is self.inv_parameters_hp
            ):
                gamma = 0
                zeta = 0
                delta = 0
            ...
            if (parameters is self.inv_parameters_lf) or (
                parameters is self.inv_parameters_hf
            ):
                gamma = 0
                zeta = 0
                delta = 0
    ...
    def write_line_parameters(self):
        ...
        nh3[:, 0] = (
            0.852 * inv_lines["gammaNH3o"].flatten() * (1e3 * spc.torr / spc.bar)
        )
        nh3[:, 1] = 1
        nh3[:, 2] = (
            0.5296 * inv_lines["gammaNH3o"].flatten() * (1e3 * spc.torr / spc.bar)
        )
        nh3[:, 3] = 1.554
        nh3[:, 4] = -0.0498
        ...
        # Inversion lines
        inv_lines = sio.loadmat(
            os.path.join(self.line_path, "nh3", "ammonia_inversion.mat")
        )
        freq = inv_lines["fo"] * 1e3
        ...
        nh3[:, 0] = (
            0.7298 * inv_lines["gammaNH3o"].flatten() * (1e3 * spc.torr / spc.bar)
        )
        nh3[:, 1] = 1
        nh3[:, 2] = (
            0.5152 * inv_lines["gammaNH3o"].flatten() * (1e3 * spc.torr / spc.bar)
        )
        nh3[:, 3] = 2 / 3
        nh3[:, 4] = -0.0627
        ...
        sio.savemat(
            os.path.join(self.lineshape_path, "NH3_inversion_DS_lowpressure.mat"), mdict
        )
        ...
        # Rotational Lines
        rot_lines = sio.loadmat(
            os.path.join(self.line_path, "nh3", "ammonia_rotational.mat")
        )
        ...
        nh3[:, 0] = (
            3.1789 * rot_lines["gNH3_rot"].flatten() * (1e3 * spc.torr / spc.bar)
        )
        nh3[:, 1] = 1
        ...
        vib_lines = sio.loadmat(
            os.path.join(self.line_path, "nh3", "ammonia_rotovibrational.mat")
        )
        ...
        mdict = {"FREQ": freq, "LGINT": lgint, "ELO": elo}
        sio.savemat(
            os.path.join(self.line_path, "nh3", "nh3_rotovibrational_DS.mat"), mdict
        )
        ...
        mdict = {"labels": labels, "NH3": nh3, "H2": h2, "He": he}
        sio.savemat(
            os.path.join(self.lineshape_path, "NH3_rotovibrational_DS.mat"), mdict
        )
        ...
        mdict = {"FREQ": freq, "LGINT": lgint, "ELO": elo}
        sio.savemat(
            os.path.join(self.line_path, "nh3", "nh3_inversion_BS_highfrequency.mat"),
            mdict,
        )
        ...
        nh3[:, 0] = (
            0.7523 * inv_lines["gammaNH3o"].flatten() * (1e3 * spc.torr / spc.bar)
        )
        nh3[:, 1] = 1
        nh3[:, 2] = (
            0.6162 * inv_lines["gammaNH3o"].flatten() * (1e3 * spc.torr / spc.bar)
        )
        nh3[:, 3] = 1.3832
        nh3[:, 4] = -0.0139
        ...
        nh3[:, 0] = (
            3.1518 * rot_lines["gNH3_rot"].flatten() * (1e3 * spc.torr / spc.bar)
        )
        nh3[:, 1] = 1
        ...
        mdict = {"FREQ": freq, "LGINT": lgint, "ELO": elo}
        sio.savemat(
            os.path.join(self.line_path, "nh3", "nh3_rotovibrational_BS.mat"), mdict
        )
        ...
        mdict = {"labels": labels, "NH3": nh3, "H2": h2, "He": he}
        sio.savemat(
            os.path.join(self.lineshape_path, "NH3_rotovibrational_BS.mat"), mdict
        )
...
class H2(molecule):
    ...
    molar_heat_capacity = (
        3.5 * spc.R
    )  # J/mol K, depends on ortho-para rate, but this is the high temperature approximation
    ...
    def __init__(self, para_ortho="equilibrium", **kwargs):
        ...
        self.cia = sio.loadmat(
            os.path.join(self.cia_path, "orton_files", "H2_absorption.mat")
        )
        super().__init__(**kwargs)
        ...
        if self.para_ortho == "equilibrium":
            self.H2H2 = spi.RectBivariateSpline(
                self.cia["wavenumber"].flatten(),
                self.cia["temperature"].flatten(),
                self.cia["H2H2_equilibrium"],
            )
            self.H2He = spi.RectBivariateSpline(
                self.cia["wavenumber"].flatten(),
                self.cia["temperature"].flatten(),
                self.cia["H2He_equilibrium"],
            )
            self.H2CH4 = spi.RectBivariateSpline(
                self.cia["wavenumber"].flatten(),
                self.cia["temperature"].flatten(),
                self.cia["H2CH4_equilibrium"],
            )
            ...
            self.molar_heat_capacity = spi.interp1d(
                temps,
                a * spc.R,
                kind="cubic",
                bounds_error=False,
                fill_value=(2.5 * spc.R, 3.5 * spc.R),
            )
        elif self.para_ortho == "normal":
            self.H2H2 = spi.RectBivariateSpline(
                self.cia["wavenumber"].flatten(),
                self.cia["temperature"].flatten(),
                self.cia["H2H2_normal"],
            )
            self.H2He = spi.RectBivariateSpline(
                self.cia["wavenumber"].flatten(),
                self.cia["temperature"].flatten(),
                self.cia["H2He_normal"],
            )
            self.H2CH4 = spi.RectBivariateSpline(
                self.cia["wavenumber"].flatten(),
                self.cia["temperature"].flatten(),
                self.cia["H2CH4_normal"],
            )
            ...
            raise ValueError(
                'Specify either "normal" or "equilibrium" para-ortho ratios'
            )
    ...
    def absorption(
        ...
        absorption = (
            number_density[gases_index.index("H2")] ** 2
            * exp(self.H2H2.ev(wavenumber, temperature))
            / self.L**2
        )

        if "He" in gases_index:
            absorption += (
                number_density[gases_index.index("H2")]
                * number_density[gases_index.index("He")]
                * exp(self.H2He.ev(wavenumber, temperature))
                / self.L**2
            )
        if "CH4" in gases_index:
            absorption += (
                number_density[gases_index.index("H2")]
                * number_density[gases_index.index("CH4")]
                * exp(self.H2CH4.ev(wavenumber, temperature))
                / self.L**2
            )
...
class CH4(molecule):
    ...
    def __init__(self, **kwargs):
        self.cia = sio.loadmat(
            os.path.join(self.cia_path, "orton_files", "CH4_absorption.mat")
        )
        super().__init__(**kwargs)
        self.CH4CH4 = spi.RectBivariateSpline(
            self.cia["wavenumber"].flatten(),
            self.cia["temperature"].flatten(),
            self.cia["CH4CH4"],
        )
    ...
    def absorption(
        ...
        number_density = gas_pressure / (spc.R * 10 * temperature) * spc.N_A
        absorption = (
            number_density[gases_index.index("CH4")] ** 2
            * exp(self.CH4CH4.ev(wavenumber, temperature))
            / self.L**2
        )
        mask = wavenumber.ravel() < self.cia["wavenumber"][:, 0]
        absorption[..., mask] = (
