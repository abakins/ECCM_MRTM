...
function alphanh3=NH3H2HeModel_quick(f,T,P,H2mr,Hemr,NH3mr,H2Omr,fo, Io, Eo, gammaNH3o, H2HeBroad,fo_rot, Io_rot, Eo_rot, gNH3_rot, gH2_rot, gHe_rot,fo_v2, Io_v2, Eo_v2,gnu_NH3,GAMMA_NH3,zeta_NH3,Z_NH3,eta_NH3,gnu_NH3_rot)
...
elseif P<=15
    ...
    gnu_H2=interp1([20,10],[1.6361,1.7465],P);  gnu_He=interp1([20,10],[0.4555,0.9779],P); gnu_NH3=0.7298;
    GAMMA_H2=interp1([20,10],[0.8,0.8202],P);   GAMMA_He=interp1([20,10],[0.5,1],P);       GAMMA_NH3=1;
    zeta_H2=interp1([20,10],[1.1313,1.2163],P); zeta_He=interp1([20,10],[0.1,0.0291],P);    zeta_NH3=0.5152;
    Z_H2=interp1([20,10],[0.6234,0.8873],P);    Z_He=interp1([20,10],[0.5,0.8994],P);       Z_NH3=2/3;
    d=interp1([20,10],[0.2,-0.0627],P);
    Con=interp1([20,10],[1.3746,0.9862],P);
end
...
gH2=gnu_H2*PH2; gHe=gnu_He*PHe; gNH3=gnu_NH3*PNH3*gammaNH3o; gH2O=gnu_H2O*PH2O;
% Broadening parameter
gamma=((gH2)*((Tdiv)^(GAMMA_H2))+(gHe)*((Tdiv)^(GAMMA_He))+gNH3*(295/T)^(GAMMA_NH3))+(gH2O)*((Tdiv)^(GAMMA_H2O));

% Shift parameter
delt=d*gamma;

% Individual coupling parameters
zH2=zeta_H2*PH2;  zHe=zeta_He*PHe; zNH3=zeta_NH3*PNH3*gammaNH3o; zH2O = zeta_H2O*PH2O;
% Coupling parameter
zeta=(zH2)*((Tdiv)^(Z_H2))+(zHe)*((Tdiv)^(Z_He))+zNH3*(295/T)^(Z_NH3)+(zH2O)*((Tdiv)^(Z_H2O));
zetasize=size(fo,1);
