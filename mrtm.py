...
def mrtm(
    ...
    if np.isclose(x_h2o, zero_ref).all():
        ...
        h2o_alpha = H2O.absorption(
            freq_ghz * 1e3, gases, gases_index, T, p_bar, units="invcm"
        )

    if np.isclose(x_nh3, zero_ref).all():
        nh3_alpha = np.zeros((len(freq_ghz), 1)) * zero_ref
    else:
        nh3_alpha = NH3.absorption(
            freq_ghz * 1e3, gases, gases_index, T, p_bar, units="invcm"
        )
    ...
    if np.isclose(x_ph3, zero_ref).all():
        ...
        ph3_alpha = PH3.absorption(
            freq_ghz * 1e3, gases, gases_index, T, p_bar, units="invcm"
        )

    # Bulk gases should never be zero
    h2_alpha = H2.absorption(
        freq_ghz * 1e3, gases, gases_index, T, p_bar, units="invcm"
    )
    he_alpha = He.absorption(
        freq_ghz * 1e3, gases, gases_index, T, p_bar, units="invcm"
    )

    alpha = np.sum(
        np.stack(
            [h2o_alpha, nh3_alpha, h2s_alpha, ch4_alpha, ph3_alpha, h2_alpha, he_alpha]
        ),
        axis=0,
    )
    ...
    # Note when trying to apply this elsewhere that altitude convention is positive _below_ the 1 bar level.
    # Contrary to usual definition for bodies with a surface
    altr = planet_radius - z_km

    k = np.max(altr) * np.sin(np.radians(incang_deg))
    total_path = sint.cumulative_trapezoid(
        (nr * altr) / np.sqrt((nr * altr) ** 2 - k**2), x=altr, initial=0
    )
    ...
    dz = pathlength
    mask = np.isfinite(dz)
    z = sint.cumulative_trapezoid(-dz[mask], initial=0.0)  # self.ray.altitude
    ...
    down_tau = -sint.cumulative_trapezoid(
        valid_alpha[..., ::-1], x=z[::-1], axis=-1, initial=0
    )[..., ::-1]
    full_tau = up_tau[..., -1]
